...
Those values belong to scene-specific mapping profiles and adapters. Japanese
Temple Pond Garden is the sole supported MVP study scene. It must expose the
normalized contract without embedding Temple-specific raw properties or
behavior in a policy, preserving the adapter boundary for possible future
scenes.
...
- Scene mappings, policies, reward logic, networking, and telemetry remain
  separate concerns.
- The Quest application stays safe and usable without cloud connectivity.# Adaptive VR Learning Research Contract
